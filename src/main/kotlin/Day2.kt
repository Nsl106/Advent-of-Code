...
data object Day2: Solution() {
    private val input by lazy { File(javaClass.getResource("2")?.toURI() ?: error("Missing input!")).readLines() }

    override fun solve(): Pair<Int, Int> {
        // Input is a List<Int> for each line
        val input = input.map { it.split(" ").map(String::toInt) }
    ...
    // Both parts
    fun originalPartTwo(): Pair<Int, Int> {
        var partOne = 0
        var partTwo = 0
        ...
        val input = input.map { it.split(" ").map { it.toInt() } }
